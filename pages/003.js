pageComponentry = {
  ...
  methods: {
    ...
    clickCorrect: function(correct){
     ...
     this.$parent.saveData('chatting-on-the-mobile.' + correct, this.pageData[correct]);
     this.activateSubmit();
    },
    ...
    loadPageData: function(){
      for(var key in this.pageData){
        if(this.pageData.hasOwnProperty(key)){
          if(this.exerciseData['chatting-on-the-mobile.' + key]){
            if(typeof this.exerciseData['chatting-on-the-mobile.' + key] === 'string'){
              this.pageData[key] = (this.exerciseData['chatting-on-the-mobile.' + key] === 'true');
            } else {
              this.pageData[key] = (this.exerciseData['chatting-on-the-mobile.' + key] === true);
            }
          }
